...
%include "src/wdc65816/tadpole.md"
%include "src/wdc65816/regAX.md"
...
static void function(Symbol f, Symbol caller[], Symbol callee[], int n)
{
    ...
    if (attr && attr->debug) print("\tpragma debug\n");
    if (attr && attr->databank) print("\tpragma databank\n");
...
static void doarg(Node p)
{
...

    {
        ...
        b = a ? a->kids[0] : NULL;
        size = opsize(p->op);
        // todo -- limit to I, U, P types.
        if (a && b 
            && (generic(a->op) == CVU  || generic(a->op) == CVI || generic(a->op) == CVP)
            && (optype(a->op) == I || optype(a->op) == U || optype(a->op) == P)
            && generic(b->op) == INDIR 
            && (optype(b->op) == I || optype(b->op) == U || optype(b->op) == P)
            && opsize(b->op) >= size) {
                // remove b -- the cast.
                p->kids[0] = b;
        }
