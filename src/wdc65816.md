...
static Symbol wdc_mkreg(int type, int offset) {
    ...
    unsigned roff = (type * 4) + offset + type;
...
static void function(Symbol f, Symbol caller[], Symbol callee[], int n)
{
    ...
    if (pascal) {
        retv = genident(REGISTER, freturn(f->type), PARAM);
        retv->defined = 1;
        retv->ref = 1;
    }
    else if (isstruct(freturn(f->type))) {
        Symbol r = wdc_mkreg(RegP, offset);

        retv = genident(REGISTER, ptr(unqual(freturn(f->type))), PARAM);
...
static void target(Node p)
{
    ...
    assert(p);
    switch (specific(p->op)) {
        case RET+U: case RET+I: case RET+P:
