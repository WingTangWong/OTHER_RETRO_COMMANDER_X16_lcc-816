...
static void progbeg(int argc, char *argv[])
{
    ...
    for (i = 0; i < 32; ++i) {
        ...
        //reg16[32+31-i] = mkreg("%%v%d", i, 1, 1);
        //reg16[64+31-i] = mkreg("%%p%d", i, 1, 2);
    }

    for (i = 0; i < 32 - 1; ++i) {
        reg32[31-i] = mkreg("%%t%d", i, 3, RegT);
        //reg32[32+31-i] = mkreg("%%v%d", i, 3, 1);
        //reg32[64+31-i] = mkreg("%%p%d", i, 3, 2);
    }

    for (i = 0; i < 32 - 3; ++i) {
        reg64[31-i] = mkreg("%%t%d", i, 7, RegT);
        //reg64[32+31-i] = mkreg("%%v%d", i, 7, 1);
        //reg64[64+31-i] = mkreg("%%p%d", i, 7, 2);
    }

    for (i = 0; i < 32 - 5; ++i) {
        reg80[31-i] = mkreg("%%t%d", i, 9, RegT);
        //reg80[32+31-i] = mkreg("%%v%d", i, 9, 1);
        //reg80[64+31-i] = mkreg("%%p%d", i, 9, 2);
    }
...
#if 0
    // need to make sure vmask is set up, first.
    if (isscalar(p->type) && !p->addressed)
